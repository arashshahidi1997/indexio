...
```bash
indexio build --config .indexio/config.yaml --root . --sources docs,notes
```

## Target a specific store

```bash
indexio build --config .indexio/config.yaml --root . --store local
```

## JSON output

```bash
indexio build --config .indexio/config.yaml --root . --json
```
